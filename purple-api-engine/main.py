            ...
            if len(lluvia_activa) >= 2 or max_rain_previo > 0.0:
                ...
                try:
                    ...
                    lambda_client.invoke(
                        FunctionName='Smability-Scheduler', # Asegúrate que este sea el nombre exacto de tu función
                        InvocationType='Event', # Asíncrono: el modelo no se queda esperando al bot
                        Payload=json.dumps({"action": "RUN_RAIN_SENTINEL"})
                    )
                    print("☔ Sentinel del Scheduler despertado con éxito.")
                except Exception as e:
                    print(f"⚠️ Error despertando al Scheduler: {e}")
